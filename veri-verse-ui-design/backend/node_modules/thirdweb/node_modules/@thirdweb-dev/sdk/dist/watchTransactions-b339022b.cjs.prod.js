...
async function getContractFromAbi(params) {
  ...
  const [resolvedAddress, {
    ...
    SmartContract
  }] = await Promise.all([index.resolveAddress(params.address), provider.getNetwork(), Promise.resolve().then(function () { return require('./smart-contract-a390a092.cjs.prod.js'); })]);
...
async function getContract(params) {
  ...
  const [resolvedAddress, {
    ...
    ContractPublisher
  }] = await Promise.all([index.resolveAddress(params.address), provider.getNetwork(), Promise.resolve().then(function () { return require('./contract-publisher-ca402ba0.cjs.prod.js'); }).then(function (n) { return n.contractPublisher; })]);
  if (inContractCache(resolvedAddress, chainId)) {
    return getCachedContract(resolvedAddress, chainId);
  }
